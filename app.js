...
app.get("/page-chefs", (req, res) => {
  //res.sendFile(__dirname + "/page-about.html");
  let sql =
    "SELECT ch.FName, ch.LName, ch.Toc, sh.ShopName FROM chefs ch, shops sh WHERE ch.shopid = sh.idShops";
...
app.get("/menu-grid", (req, res) => {
  ...
  let sql =
    "select bf.ItemName,bf.Amount,bf.Details, ch.Fname from breakfast bf, chefs ch where (ch.idChefs = bf.idChefs);";
  ...
  let sql2 =
    "select ln.ItemName,ln.Amount,ln.Details, ch.Fname from lunch ln, chefs ch where (ch.idChefs = ln.idChefs);";
  ...
  let sql3 =
    "select dn.ItemName,dn.Amount,dn.Details, ch.Fname from dinner dn, chefs ch where (ch.idChefs = dn.idChefs);";
  ...
  let sql4 =
    "select dsr.ItemName,dsr.Amount,dsr.Details, ch.Fname from dessert dsr, chefs ch where (ch.idChefs = dsr.idChefs);";
  ...
  let sql5 =
    "select dr.ItemName,dr.Amount,dr.Details, ch.Fname from drinks dr, chefs ch where (ch.idChefs = dr.idChefs);";
  ...
  sql =
    "select bf.ItemName,bf.Amount,bf.Details from breakfast bf, todaymenu tm where (bf.idBreakfast = tm.idBreakfast);";
  ...
  sql =
    "select lu.ItemName,lu.Amount,lu.Details from lunch lu, todaymenu tm where (lu.idlunch = tm.idlunch);";
  ...
  sql =
    "select di.ItemName,di.Amount,di.Details from dinner di, todaymenu tm where (di.iddinner = tm.iddinner);";
  ...
  sql =
    "select de.Itemname,de.Amount,de.Details from dessert de, todaymenu tm where (de.iddessert = tm.iddessert);";
  ...
  sql =
    "select dr.ItemName,dr.Amount,dr.Details from drinks dr, todaymenu tm where (dr.idDrinks = tm.idDrinks);";
